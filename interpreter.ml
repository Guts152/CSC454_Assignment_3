...
and interpret_do (loop_count:int) (sl:ast_sl) (mem:memory)
    (inp:string list) (outp:string list)
  : status * memory * string list * string list =
  interpret_do_helper loop_count sl sl (new_scope mem) inp outp
...
and interpret_do_helper loop_count remaining_sl original_sl mem inp outp =
    ...
    let (mem, outp) = (new_mem, new_outp) in
    (Good, mem, new_inp, outp)
...
let primes_parse_tree = parse ecg_parse_table primes_prog1;;
...
let sqrt_prog_parse_tree = parse ecg_parse_table sqrt_prog1;;
let sqrt_prog_syntax_tree = ast_ize_prog sqrt_prog_parse_tree;;

let float_trunc_parse_tree = parse ecg_parse_table float_trunc;;
let float_trunc_syntax_tree = ast_ize_prog float_trunc_parse_tree;;
...
let main () =
  ...
  print_string (interpret gcd_syntax_tree "10 15");
    (* should print "5" *)
  print_newline ();
  print_string (interpret float_trunc_syntax_tree "");
