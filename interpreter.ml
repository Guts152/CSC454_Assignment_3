...
let rec lookup_mem (id:string) (loc:row_col) (mem:memory) : value =
  ...
  let rec lookup_mem_scope (id:string) (loc:row_col) (mem:memory) : value =
    match mem with
    | [] -> Error (complaint loc (id ^ " not found"))
    | scope :: surround ->
        match find_opt (name_match id) scope with
        | Some (_, v) -> v
        | None -> Error (complaint loc (id ^ " not in this scope"));;
...
and interpret_dec (iter1:bool) (id:string) (v:value) (vloc:row_col)
  ...
  match lookup_mem_scope id vloc mem with
  | Error _ -> 
...
and interpret_do (loop_count:int) (sl:ast_sl) (mem:memory)
  ...
  match status with
  | Done -> (Good, (end_scope new_mem), new_inp, new_outp)
...
let (status, new_mem, new_inp, new_outp, max_count) = interpret_sl loop_count (loop_count = loop_min) sl (new_scope mem) inp outp in 
    match status with
    | Bad -> (Bad, new_mem, new_inp, new_outp)(* Error handling *)
    | Good ->
...
let trunc_parse_tree = parse ecg_parse_table trunc_prog;;
